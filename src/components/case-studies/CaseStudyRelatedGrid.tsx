              ...
              <div className="w-full flex flex-col gap-6 md:gap-[43.214px]">
                <div className="w-full flex flex-col gap-3 md:gap-[13.604px]">
                  {/* <div
                    className={
                      isLight
                        ? "text-[32px] sm:text-[38px] md:text-[44.815px] leading-[1.15] md:leading-[1.3] font-bold text-black"
                        : "text-[32px] sm:text-[38px] md:text-[44.815px] leading-[1.15] md:leading-[1.3] font-bold text-white"
                    }
                  >
                    {(() => {
                      const words = cs.title.split(" ");
                      const lastWord = words.pop();

                      return (
                        <>
                          {words.join(" ")}{" "}
                          <span className="text-[#E21F26]">{lastWord}</span>
                        </>
                      );
                    })()}
                  </div> */}
                  <Link href={cs.href} className={isLight ? "text-[16px] sm:text-[18px] md:text-[22.577px] leading-[1.4] md:leading-[1.3] font-light text-[#787878] w-full max-w-[616.06px] hover:text-[#E21F26]" : "text-[16px] sm:text-[18px] md:text-[22.577px] leading-[1.4] md:leading-[1.3] font-light text-[#BFBFBF] w-full max-w-[616.06px] hover:text-[#E21F26]"}>
                    {cs.description}
                  </Link>
                </div>
