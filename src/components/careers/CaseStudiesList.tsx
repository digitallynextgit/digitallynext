...
export default function CaseStudiesList({ theme }: CaseStudiesListProps) {
  ...
  const buildHighlightedParts = (text: string, keywords: string[]) => {
    ...
    const escaped = [...keywords].sort((a, b) => b.length - a.length).map(escapeRegExp);
    ...
    return fields.slice(0, 3).map((field) => (
      <div
        ...
        className={[
          ...
          isDark ? 'border-[#2a2a2a] text-white' : 'border-[#D3D3D3] text-black',
        ].join(' ')}
      >
        <Image
            ...
            {list.map((cs, index) => (
              <Link
                ...
                className={[
                  ...
                  isDark ? 'hover:bg-[#111111] border-[#2a2a2a]' : 'hover:bg-[#F5F5F5] border-[#E5E5E5]',
                  isDark ? 'border-t border-[#2a2a2a]' : 'border-t border-[#E5E5E5]',
                  index === list.length - 1 ? (isDark ? 'border-b border-[#2a2a2a]' : 'border-b border-[#E5E5E5]') : '',
                ].join(' ')}
              >
                {/* Text */}
                <p
                ...
                >
                  {renderHighlightedText(cs.listing.caption, cs.listing.highlightKeywords ?? [])}
                </p>
