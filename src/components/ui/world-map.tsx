...
export default function WorldMap({ dots = [], lineColor = '#0ea5e9', theme = 'light' }: MapProps) {
    ...
    <div className={`w-full aspect-2/1 rounded-lg relative font-sans ${isDark ? 'bg-black' : 'bg-[#F5F5F5]'}`}>
      ...
      >
        ...
        {dots.map((dot, i) => {
          ...
          const isSamePoint = dot.start.lat === dot.end.lat && dot.start.lng === dot.end.lng;
          ...
          return points.map((pt, j) => (
            <g key={`dot-${i}-${j}`}>
              ...
              <circle cx={pt.x} cy={pt.y} r="1" fill={color} opacity="0.5">
                <animate attributeName="r" from="1" to="4" dur="1.5s" begin="0s" repeatCount="indefinite" />
